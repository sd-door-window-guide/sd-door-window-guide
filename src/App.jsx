...
const buildQuestions = () => [
  ...
  { id:"coastalProximity", section:"Property Details", question:"Is your home within 2 miles of the ocean or bay?", subtitle:"Salt air accelerates corrosion on hardware and frames — product selection differs significantly near the coast", isCoastal:true, options:[{value:"yes_beachfront",label:"Beachfront / Direct Ocean",icon:"",detail:"Right on the water — maximum salt air exposure"},{value:"yes_near",label:"Within 1 Mile",icon:"",detail:"Strong salt air influence — coastal spec required"},{value:"yes_2mile",label:"1–2 Miles",icon:"",detail:"Moderate salt air — coastal-grade recommended"},{value:"no",label:"Inland — Over 2 Miles",icon:"",detail:"Minimal salt air concern"},{value:"unsure",label:"Not Sure",icon:"",detail:"I'm not certain of the distance"}] },
  ...
  return (
    <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:"12px",overflow:"hidden",borderTop:`3px solid ${c}`}}>
      {vendor.heroImage&&<div style={{width:"100%",height:"180px",overflow:"hidden",position:"relative"}}>
        ...
        <div style={{position:"absolute",inset:0,background:`linear-gradient(to bottom, transparent 40%, ${T.card}dd 100%)`}}/>
      </div>}
            ...
            <div key={`q${step}`} style={{animation:"fadeUp 0.22s ease"}}>
              ...
              {currentQ.isPermits&&(
                <div style={{background:"#EEF2FA",border:`1px solid ${T.slate}44`,borderRadius:"8px",padding:"14px 16px",marginBottom:"18px"}}>
                  <div style={{fontSize:"9px",letterSpacing:"3px",color:T.slate,marginBottom:"10px",fontFamily:"monospace"}}> TITLE 24 {"—"} CALIFORNIA ENERGY CODE</div>
                  <p style={{fontSize:"13px",color:"#4A6898",lineHeight:1.65,margin:"0 0 12px"}}>Any permitted window or door replacement in California must comply with <strong style={{color:"#aabbd4"}}>Title 24, Part 6</strong> {"\u2014"} the California Building Energy Efficiency Standards. This is enforced at inspection. The 2025 Title 24 Standards (effective January 1, 2026) tightened requirements across the state.</p>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:"8px",marginBottom:"12px"}}>
                    {[
                      {label:"U-Factor",icon:"",desc:"Measures how much heat passes through the entire window assembly (frame + glass). Lower is better. The 2026 standard requires U-0.30 or better for most climate zones in San Diego County. U-factor is the primary insulation measure — critical for keeping interior temperatures stable."},
                      {label:"SHGC",icon:"",desc:"Solar Heat Gain Coefficient — measures how much solar heat passes through the glass, on a scale of 0 to 1. Lower SHGC = less solar heat entering. San Diego's 2026 requirement is typically SHGC 0.23 or less for west and south-facing windows in Climate Zones 7 and 10. Reduces air conditioning load."},
                      {label:"Climate Zone 7",icon:"",desc:"Covers coastal San Diego — La Jolla, Del Mar, Encinitas, Chula Vista, Coronado. Moderate climate. U-0.30, SHGC 0.23 typically required on permitted replacement projects."},
                      {label:"Climate Zone 10",icon:"",desc:"Inland San Diego — El Cajon, Santee, Escondido, Poway, Rancho Bernardo. Hotter summers. SHGC requirements are stricter — solar heat gain is a bigger concern than in coastal zones."},
                    ].map(item=>(
                      <div key={item.label} style={{background:"#EEF2FA",borderRadius:"6px",padding:"10px 12px",borderLeft:`2px solid ${T.slate}55`}}>
                        <div style={{fontSize:"12px",fontWeight:500,color:"#4A6898",marginBottom:"4px"}}>{item.icon} {item.label}</div>
                        <div style={{fontSize:"10px",color:"#3a4a60",lineHeight:1.5}}>{item.desc}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{background:`${T.slate}10`,border:`1px solid ${T.slate}33`,borderRadius:"6px",padding:"10px 13px",marginBottom:"8px"}}>
                    <div style={{fontSize:"9px",letterSpacing:"2px",color:T.slate,fontFamily:"monospace",marginBottom:"4px"}}>2026 UPDATES {"\u2014"} WHAT CHANGED</div>
                    <div style={{fontSize:"11px",color:"#2A4A70",lineHeight:1.6}}>The 2025 Title 24 Standards (effective Jan 1, 2026) lowered the maximum U-factor from 0.32 to <strong style={{color:"#7a8aaa"}}>0.30</strong> for most replacement windows in San Diego County, and tightened SHGC requirements in inland zones. Many older window products that were Title 24 compliant before 2026 no longer qualify {"\u2014"} confirm your product's compliance certificate with your supplier before ordering on a permitted job.</div>
                  </div>
                  <div style={{fontSize:"11px",color:"#3a4858",lineHeight:1.5}}>Your specialist will confirm the correct Title 24 requirements for your specific climate zone, project type, and window orientation. All products available through this guide carry current compliance documentation.</div>
                </div>
              )}
              ...
              {currentQ.isCoastal&&(
                <div style={{background:"#EEF2FA",border:`1px solid ${T.teal}44`,borderRadius:"8px",padding:"14px 16px",marginBottom:"18px"}}>
                  <div style={{fontSize:"9px",letterSpacing:"3px",color:T.teal,marginBottom:"8px",fontFamily:"monospace"}}> WHY COASTAL PROXIMITY MATTERS</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:"8px",marginBottom:"10px"}}>
                    {[
                      {label:"Hardware Corrosion",desc:"Standard zinc or brass hardware corrodes rapidly within 1–2 miles of the ocean. Salty air oxidizes exposed metal. Coastal-rated stainless steel or marine-grade hardware is required — Marvin's coastal stainless program, NanaWall coastal hardware, and Milgard's corrosion-resistant hardware packages are the right call."},
                      {label:"Frame Material",desc:"Aluminum frames within direct ocean exposure should be anodized or powder-coated with a marine-grade finish. Wood frames within a mile of the ocean require diligent maintenance. Fiberglass (Marvin Ultrex®) and clad-wood with aluminum exterior are the most corrosion-resistant frame options for oceanfront properties."},
                      {label:"Glass Seals",desc:"Salt air can degrade standard silicone sealants over time. Coastal installations should use marine-grade sealants and flashing. IGU edge seals also benefit from higher-quality spacer systems (foam spacer vs. aluminum spacer) which reduce thermal bridging and moisture ingress."},
                      {label:"Warranty Implications",desc:"Most window manufacturers void or reduce their warranty if their standard product is installed within a specified distance of salt water without the coastal upgrade package. Always confirm coastal warranty coverage before specifying — especially for La Jolla, Del Mar, Coronado, and Encinitas beachfront properties."},
                    ].map(item=>(
                      <div key={item.label} style={{background:"#EEF4FA",borderRadius:"6px",padding:"10px 12px",borderLeft:`2px solid ${T.teal}44`}}>
                        <div style={{fontSize:"11px",fontWeight:500,color:"#5a9a96",marginBottom:"3px"}}>{item.label}</div>
                        <div style={{fontSize:"10px",color:"#2a4a46",lineHeight:1.5}}>{item.desc}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{fontSize:"11px",color:"#2a4a46",lineHeight:1.5}}>Your specialist will recommend the appropriate coastal package for your distance from the water. This is not optional for beachfront properties {"\u2014"} it directly affects product longevity and warranty coverage.</div>
                </div>
              )}
              ...
              {currentQ.isElevation&&(
                <div style={{background:"#EEF5EE",border:`1px solid ${T.sage}44`,borderRadius:"8px",padding:"14px 16px",marginBottom:"18px"}}>
                  <div style={{fontSize:"9px",letterSpacing:"3px",color:T.sage,marginBottom:"8px",fontFamily:"monospace"}}> HIGH ELEVATION &amp; INSULATED GLASS UNITS</div>
                  <p style={{fontSize:"13px",color:"#7a9a7a",lineHeight:1.65,margin:"0 0 10px"}}>Insulated glass units (IGUs) are factory-sealed with air or gas between the panes at the manufacturer's elevation — typically near sea level. When these units are installed at significantly higher elevations, the pressure differential between the sealed unit and the outside atmosphere causes the glass to bow, flex, and in some cases fail prematurely.</p>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:"8px",marginBottom:"10px"}}>
                    {[
                      {label:"Breather Tubes",desc:"A capillary tube or breather tube is inserted into the IGU spacer before sealing, allowing the unit to equalize pressure during transport and after installation at altitude. Once installed and pressure-equalized, the tube is sealed. This is the standard solution for high-elevation IGU installations."},
                      {label:"Affected Areas",desc:"Julian (4,235 ft), Pine Valley (~3,900 ft), Mt. Laguna (~6,000 ft), Alpine (~1,950 ft — borderline), Descanso (~3,500 ft). If your project is in the Julian / Cuyamaca / Laguna Mountains area, breather tubes are required on all IGU orders."},
                      {label:"What Happens Without It",desc:"Without pressure equalization, IGU units at elevation can develop convex bowing of the glass panes, stress on the edge seals leading to premature seal failure, fogging between the panes, and in severe cases, spontaneous glass breakage. Void most manufacturer warranties if not specified."},
                      {label:"Cost Impact",desc:"Breather tubes add a modest cost — typically $15–$40 per IGU unit depending on size. A nominal expense relative to the cost of premature unit failure. All premium manufacturers (Andersen, Marvin, Milgard, NanaWall) offer altitude-rated units with breather tubes on request."},
                    ].map(item=>(
                      <div key={item.label} style={{background:"#EEF5EE",borderRadius:"6px",padding:"10px 12px",borderLeft:`2px solid ${T.sage}44`}}>
                        <div style={{fontSize:"11px",fontWeight:500,color:"#5a7a5a",marginBottom:"3px"}}>{item.label}</div>
                        <div style={{fontSize:"10px",color:"#2a3a2a",lineHeight:1.5}}>{item.desc}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{fontSize:"11px",color:"#2a3a2a",lineHeight:1.5}}>If you're in a high-elevation area, make sure to flag this with your specialist before your order is placed. Breather tubes must be specified at the factory — they cannot be added after manufacturing.</div>
                </div>
              )}
              {currentQ.isFireZone&&(
